...
Sergey Ioffe and Christian Szegedy, "Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift", ICML 2015.
  ...
  if mode == 'train':
    ...
    #############################################################################
        ...
        diff_sqrd = x_minus_mean**2

        # Step 4 - shape of var (D,)
        var = 1 / float(N) * np.sum(diff_sqrd, axis=0)
        ...
        cache = (mean, x_minus_mean, diff_sqrd, var, sqrt_var, inv_var,
                 x_hat, gamma_xhat, gamma, beta, x, bn_param,eps)
...
def batchnorm_backward_alt(dout, cache):
  ...
  """
  dx, dgamma, dbeta = None, None, None
  mean, x_minus_mean, diff_sqrd, var, sqrt_var, inv_var,x_hat, gamma_xhat, gamma, beta, x, bn_param,eps=cache
  #############################################################################
  # TODO: Implement the backward pass for batch normalization. Store the      #
  # results in the dx, dgamma, and dbeta variables.                           #
  #                                                                           #
  ...
  dvar=np.sum(gamma*x_minus_mean*(-0.5)*((var+eps)**-3/2),axis=0)
  dmean=np.sum(gamma*(-inv_var),axis=0)+dvar*(np.sum(-2*x_minus_mean,axis=0)/dout.shape[0])
  dx=(gamma*inv_var)+dvar*(2*x_minus_mean)/dout.shape[0]+dmean/dout.shape[0]	
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def conv_backward_naive(dout, cache):
  ...
  dx= np.empty((N, C, H+2*pad, W+2*pad))
  dw= np.empty((F, C * HH * WW))
  db= np.empty((F,))
